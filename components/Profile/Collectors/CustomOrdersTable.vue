<template>
  <div class="orders-table">
    <table class="table">
      ...
      <tbody v-if="orders.length > 0">
        <tr v-for="order in orders" :key="order.id">
          <td>
            <nuxt-link :to="`/artists/${order.artist.id}`">
              {{
                order.artist.first_name + ' ' + order.artist.last_name
              }}</nuxt-link
            >
          </td>
    ...
    <div v-if="orders.length < 1" class="text-center no-content">
      ...
      <h3>
        Oops! No custom orders requested yet, click the button below to request
        one.
      </h3>
    </div>
    <div class="text-center mb-2">
      <nuxt-link :to="`/custom-orders`" class="btn btn-sm btn-primary">
...
export default {
  name: 'CollectorCustomOrdersTable',
  filters: {
  ...
  methods: {
    async getMyOrders(id) {
      try {
        const resp = await this.$axios.get(`/custom-orders/?requested_by=${id}`)
